...
const Nav = class extends React.Component {
  ...
  render() {
    ...
    <nav className="lg:flex lg:content-end lg:justify-end lg:flex-col lg:pr-8 nav">
      <div className={`bg-red md:bg-none fixed top-0 left-0 z-40 w-full h-screen p-6 transition-all duration-100 ease-in md:relative md:top-auto md:left-auto md:block md:w-full md:h-auto md:p-0 md:bg-transparent text-white ${menuActive}`} >
        <ul className="text-3xl md:bg-none md:flex md:w-full md:flex-row md:justify-start md:items-stretch md:text-sm xl:text-lg">
          ...
          <li>
            <Link className="block py-2 font-bold md:text-white md:px-3 text-sugar-pink hover:text-sugar-pink" onClick={this.toggleMenu} activeClassName="is-active" to="/sustainability/">Sustainability</Link>
          </li>
          <li>
      ...
      <div aria-label="Navigation menu button" tabIndex={0} role="button" className={`${burgerActive} fixed bottom-0 right-0 z-50 block text-white cursor-pointer bg-red md:hidden p-4`} onClick={this.toggleMenu} onKeyDown={this.toggleMenu}>
